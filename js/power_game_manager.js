...
PowerGameManager.prototype.updateStatus = function () {
  ...
  this.hardStalemate = !this.won && !this.canMergeEventuallyForMode(board, this.mode || "classic");
};

PowerGameManager.prototype.canMergeEventuallyForMode = function (start, mode) {
  if (mode === "push") return this.shortestSolutionForMode(start, mode) > 0;
  var original = this.mode;
  this.mode = mode;
  var result = this.canMergeEventually(start);
  this.mode = original;
  return result;
};
